...
class queryFactory {
  ...
  function ExecuteRandomMulti($zf_sql, $zf_limit = 0, $zf_cache = false, $zf_cachetime=0) {
    ...
    $this->count_queries++;
    return($obj);
  }/* EOF:function ExecuteRandomMulti */

  function insert_ID() { /* 直近の INSERT 操作で生成された ID を得る */
    return @mysql_insert_id($this->link);
  }/* EOF:function insert_ID */

  function metaColumns($zp_table) {
    $res = @mysql_query("select * from " . $zp_table . " limit 1", $this->link);
    $num_fields = @mysql_num_fields($res);/* 結果におけるフィールドの数を得る */
    for ($i = 0; $i < $num_fields; $i++) {
     $obj[strtoupper(@mysql_field_name($res, $i))] = new queryFactoryMeta($i, $res);
    }
    return $obj;
  }/* EOF:function metaColumns */

  function get_server_info() { /* MySQL サーバ情報を取得する */
    if ($this->link) {
      return mysql_get_server_info($this->link);
    } else {
      return UNKNOWN;
    }
  }/* EOF:function get_server_info */

  function queryCount() {
    return $this->count_queries;
  }/* EOF:function queryCount */

  function queryTime() {
    return $this->total_query_time;
  }/* EOF:function queryTime */
...
**/
  function bindVars($sql, $bindVarString, $bindVarValue, $bindVarType, $debug = false) {
    ...
    return $sqlNew;
  }/* EOF:function bindVars */

  function prepareInput($string) {
    return @mysql_real_escape_string($string);
  }/* EOF:function prepareInput */

}/* EOF:class queryFactory */
/************************* EOF: QUERY FACTORY *************************************/

/************************* BOF: QUERY FACTORY RESULT *************************************/
...
}/* EOF:class queryFactoryMeta */

/************************* EOF: QUERY FACTORY RESULT *************************************/
